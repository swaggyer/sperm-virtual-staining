...
class _DSConvPooling(nn.Module):
    def __init__(self, dw_channels, out_channels, stride=1):
        super(_DSConvPooling, self).__init__()
        self.conv = nn.Sequential(
...
class DSPool_ECA(nn.Module):
    def __init__(self,in_channels,out_channels):
        super(DSPool_ECA,self).__init__()
        ...
        self.down_conv2 = _DSConvPooling(dw_channels=32,out_channels=64)
        self.down_conv3 = _DSConvPooling(dw_channels=64,out_channels=128)
        self.down_conv4 = _DSConvPooling(dw_channels=128,out_channels=256)
...
class DSPool_ECA_DOWN(nn.Module):
    def __init__(self,in_channels,out_channels):
        super(DSPool_ECA_DOWN,self).__init__()
        self.down_conv1 = _ConvBNReLU(in_channels=in_channels,out_channels=32,kernel_size=3,stride=1,padding=1,padding_mode = "reflect")
        self.down_conv2 = _DSConvPooling(dw_channels=32,out_channels=64)
        self.down_conv3 = _DSConvPooling(dw_channels=64,out_channels=128)
        self.down_conv4 = _DSConvPooling(dw_channels=128,out_channels=256)
